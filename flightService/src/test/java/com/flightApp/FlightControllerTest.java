...
public class FlightControllerTest {
    ...
    void searchFlights_ShouldReturnFlights() {
        ...
        Mockito.when(flightService.searchFlights(any(FlightSearchRequest.class)))
    ...
    @Test
    void searchFlights_InvalidInput_ShouldReturn400() {
        String invalidJson = "{ \"fromPlace\": \"Mars\" }";
    ...
    void addInventory_Success() {
        ...
        response.setAirlineName("Vistara");

        Mockito.when(flightService.addFlight(any())).thenReturn(Mono.just(response));
        ...
        webTestClient.post()
                ...
                .expectStatus().isOk()
                .expectBody()
                .jsonPath("$.flightId").isEqualTo("999");
    }
    ...
    void getFlightById_Success() {
        ...
        Mockito.when(flightService.getFlightById("101")).thenReturn(Mono.just(response));
    ...
    void getFlightById_NotFound_ShouldReturn404() {
        Mockito.when(flightService.getFlightById("unknownId"))
                .thenThrow(new ResourceNotFoundException("Flight not found"));

        webTestClient.get()
                .uri("/flight/unknownId")
                .exchange()
    ...
    @Test
    void addInventory_ValidationFailure_ShouldHitWebExchangeBindException() {
        webTestClient.post()
                .uri("/flight/airline/inventory")
                .contentType(MediaType.APPLICATION_JSON)
                .bodyValue("{}") 
    ...
    @Test
    void getFlight_NotFound_ShouldReturn404() {
        Mockito.when(flightService.getFlightById("999"))
               .thenThrow(new ResourceNotFoundException("Flight not found"));
    ...
    void addInventory_DuplicateFlight_ShouldReturn409() {
        Mockito.when(flightService.addFlight(any()))
               .thenThrow(new org.springframework.dao.DuplicateKeyException("Duplicate flight"));
    ...
    void updateFlight_ConcurrencyError_ShouldReturnConflict() {
        ...
        webTestClient.post()
                ...
                .expectStatus().is4xxClientError()
                .expectBody()
                .jsonPath("$.message").isEqualTo("The data has been updated by another user. Please refresh and try again.");
    }
    @Test
